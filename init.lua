...
vim.keymap.set("n", "<leader>_", ":split<CR>", { desc = "Split window vertically" })
	...
	},
	{
		...
		config = function()
			...
			vim.keymap.set("n", "<leader>u", function()
				...
				local undotree_bufname = "undotree"
				local windows = vim.api.nvim_tabpage_list_wins(0) -- List all windows in the current tab
				for _, win in ipairs(windows) do
					local bufname = vim.fn.bufname(vim.api.nvim_win_get_buf(win))
					if bufname:match(undotree_bufname) then
						vim.api.nvim_set_current_win(win)
						break
					end
				end
			end, { desc = "Toggle undotree and focus on it" })

			-- Persistent undo directory setup
			if vim.fn.has("persistent_undo") == 1 then
				local target_path = os.getenv("HOME") .. "/.config/nvim/undodir"
